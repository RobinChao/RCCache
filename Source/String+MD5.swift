...
class HashBase {
    ...
    func prepare(len: Int = 64) -> NSMutableData {
        let result = NSMutableData(data: self.message)
        
        // Step 1: Append Padding Bits
        result.appendBytes([0x80]) // append one bit (UInt8 with one bit) to result
        
         // append "0" bit until result length in bits = 448 (mod 512)
        var length =  result.length
        var counter = 0
        
        while length % len != (len - 8) {
            counter++
            length++
        }
        
        let bufZeros = UnsafeMutablePointer<UInt8>(calloc(counter, sizeof(UInt8)))
        result.appendBytes(bufZeros, length: counter)
